...
var ABuffer = require('safe-buffer').Buffer;
...
res.send = function send(body) {
  ...
  switch ((0, _typeof2["default"])(chunk)) {
    ...
    case 'object':
      if (chunk === null) {
        chunk = '';
      } else if (ABuffer.isBuffer(chunk)) {
        if (!this.get('Content-Type')) {
          this.type('bin');
        }
      } else {
  ...
  if (chunk !== undefined) {
    if (ABuffer.isBuffer(chunk)) {
      // get length of Buffer
      len = chunk.length;
    } else if (!generateETag && chunk.length < 1000) {
      // just calculate length when no ETag + small chunk
      len = ABuffer.byteLength(chunk, encoding);
    } else {
      // convert chunk to Buffer and calculate
      chunk = ABuffer.from(chunk, encoding);
      encoding = undefined;
      len = chunk.length;
    }
...
res.cookie = function (name, value, options) {
  ...
  var val = (0, _typeof2["default"])(value) === 'object' ? 'j:' + JSON.stringify(value) : String(value);
...
res.redirect = function redirect(url) {
  ...
  this.set('Content-Length', ABuffer.byteLength(body));
